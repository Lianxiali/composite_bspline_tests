                   ...
                   void* /*ctx*/)
{
    ...
    if (struct_list[strct_num].compare("curve2d") == 0)
    {
        ...
        ds[strct_num] = mfac*dx/R;  // angle = L/R
    ...
    {
		double mfac = 0.025; // small number to set dense markers
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        f_tracer.unsetf(ios_base::showpos);
        f_tracer.setf(ios_base::scientific);
        f_tracer.precision(15);
        std::string ib_delta_function = input_db->getString("DELTA_FUNCTION");
        std::ofstream f_area(ib_delta_function + "tracer.area");
        f_area << "CurrentTime " << " area" << std::endl;
        f_area.unsetf(ios_base::showpos);
        f_area.setf(ios_base::scientific);
        f_area.precision(15);
#pragma region get Lagraingian points of the tracer structure
    ...
    const std::string structure_name = "tracer";
    PassiveTracers tracers(structure_name, loop_time, f_tracer, finest_ln, l_data_manager);
    double area = tracers.getArea();
    f_area << loop_time << " " << area << std::endl;
    tracers.outputTracerPositions();
#pragma endregion
