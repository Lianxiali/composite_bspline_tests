...
generate_structure(const unsigned int& strct_num,
                   ...
                   void* /*ctx*/)
{
    pout << "struct_num = " << strct_num << std::endl;
    ...
    if (struct_list[strct_num].compare("curve2d") == 0)
    {
        ...
        // ds[strct_num] = mfac * dx;
        // Cole defines mfac as follows: mfac = DX/h
        // DX = r*ds so ds = mfac*h/r .
        ds[strct_num] = mfac*dx/R;
		
        num_node[strct_num] = ceil(2.0 * M_PI / ds[strct_num]);
        num_vertices = num_node[strct_num];
        vertex_posn.resize(num_vertices);
        double dsita = ds[strct_num] / R;
        for (std::vector<IBTK::Point>::iterator it = vertex_posn.begin(); it != vertex_posn.end(); ++it)
        {
            ...
            tracer_positions.emplace_back(X(0), X(1));
        }
    }
    else if (struct_list[strct_num].compare("tracer") == 0)
    {
		double mfac = 50; // small number to set dense markers
        ds[strct_num] = mfac*dx/R;
    ...
    void* /*ctx*/)
{
    ...
    if (struct_list[strct_num].compare("curve2d") == 0)
    {
        for (int k = 0; k < num_node[strct_num]; ++k)
        {
            ...
            // pout << "ds = " << ds[strct_num] << " spring constant = " << parameters[0] << endl;
    ...
    else if (struct_list[strct_num].compare("tracer") == 0)
    {
        for (int k = 0; k < num_node[strct_num]; ++k)
        {
            ...
            spring_map.insert(std::make_pair(e.first, e));
            IBRedundantInitializer::SpringSpec spec_data;
            parameters[0] = 0.0; // spring constant
            parameters[1] = 0.0;               // resting length
            spec_data.parameters = parameters;
            spec_data.force_fcn_idx = force_fcn_idx;
            spring_spec.insert(std::make_pair(e, spec_data));
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        if (rank == 0)
        {
            ftracer.open("IBTracerCoordinates.csv");
        }        
        ...
        visit_data_writer->registerPlotQuantity("P_error", "SCALAR", p_cloned_idx);
...
const std::string structure_name = "curve2d";
const int structure_id = l_data_manager->getLagrangianStructureID(structure_name, finest_ln);
std::pair<int, int> lag_idx_range = l_data_manager->getLagrangianStructureIndexRange(structure_id, finest_ln);
...
Pointer<LData> ptr_x_lag_data = l_data_manager->getLData("X", finest_ln);
...
for (const auto& node : local_nodes)
{
    ...
    if (lag_idx_range.first <= lag_idx && lag_idx < lag_idx_range.second)
    {
        ...
        std::cout << "Marker " << local_idx << " Position: (";
        std::cout << X[0] << "," << X[1];
        std::cout << ")" << std::endl;
    }
}
...
#pragma region write lagragian points to a csv file
            if(use_markers)
            {
                Pointer<IBExplicitHierarchyIntegrator> explicit_time_integrator = time_integrator;
                ...
                if (rank == 0)
                {
                    ftracer.unsetf(ios_base::showpos);
                    ftracer.setf(ios_base::scientific);
                    ftracer.precision(15);

                    for (const auto pt : tracer_positions)
                    {
                        ftracer << loop_time << ", " << pt(0) << ", " << pt(1) << "\n";
                    }
                }
            }
#pragma endregion
            ...
            const string& data_dump_dirname)
{
    ...
    l_data_manager->scatterPETScToLagrangian(X_petsc_vec, X_lag_vec, finest_ln);
